import React, { useId } from 'react';
import './SellerSalesStatisticsMetricCard.css';

function SellerSalesTrendChart({ gradientId, tone = 'neutral' }) {
  const palettes = {
    positive: {
      stroke: '#16a34a',
      fill: '#86efac',
      areaPath:
        'M0 36 C10 35 18 28 28 30 C38 32 48 22 58 24 C68 26 78 16 88 18 C98 20 108 12 120 10 L120 48 L0 48 Z',
      linePath:
        'M0 36 C10 35 18 28 28 30 C38 32 48 22 58 24 C68 26 78 16 88 18 C98 20 108 12 120 10',
      dotY: 10,
    },
    negative: {
      stroke: '#dc2626',
      fill: '#fca5a5',
      areaPath:
        'M0 12 C10 14 18 18 28 20 C38 22 48 24 58 26 C68 28 78 30 88 32 C98 34 108 35 120 37 L120 48 L0 48 Z',
      linePath:
        'M0 12 C10 14 18 18 28 20 C38 22 48 24 58 26 C68 28 78 30 88 32 C98 34 108 35 120 37',
      dotY: 37,
    },
    neutral: {
      stroke: '#2563eb',
      fill: '#93c5fd',
      areaPath:
        'M0 24 C12 24 24 23 36 24 C48 25 60 24 72 24 C84 24 96 23 108 24 C114 24 118 24 120 24 L120 48 L0 48 Z',
      linePath:
        'M0 24 C12 24 24 23 36 24 C48 25 60 24 72 24 C84 24 96 23 108 24 C114 24 118 24 120 24',
      dotY: 24,
    },
  };

  const palette = palettes[tone] || palettes.neutral;

  return (
    <svg viewBox="0 0 120 48" fill="none" preserveAspectRatio="none" aria-hidden="true">
      <defs>
        <linearGradient id={gradientId} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={palette.fill} stopOpacity="0.42" />
          <stop offset="100%" stopColor={palette.fill} stopOpacity="0" />
        </linearGradient>
      </defs>
      <path d={palette.areaPath} fill={`url(#${gradientId})`} />
      <path
        d={palette.linePath}
        stroke={palette.stroke}
        strokeWidth="2.5"
        fill="none"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <circle cx="120" cy={palette.dotY} r="3.5" fill={palette.stroke} />
    </svg>
  );
}

export default function SellerSalesStatisticsMetricCard({
  title,
  value,
  growthFormatted = '0%',
  tone = 'neutral',
  loading = false,
}) {
  const gradientId = useId().replace(/:/g, '');

  return (
    <article className="seller-sales-statistics-metric-card">
      <div className="seller-sales-statistics-metric-card__header">
        <h3 className="seller-sales-statistics-metric-card__title">{title}</h3>
        <span
          className={`seller-sales-statistics-metric-card__badge seller-sales-statistics-metric-card__badge--${tone}`}
        >
          {loading ? '...' : growthFormatted}
        </span>
      </div>

      <div className="seller-sales-statistics-metric-card__body">
        <div className="seller-sales-statistics-metric-card__value-row">
          <p className="seller-sales-statistics-metric-card__value">{loading ? '...' : value}</p>
          {!loading ? (
            <span
              className={`seller-sales-statistics-metric-card__inline-growth seller-sales-statistics-metric-card__inline-growth--${tone}`}
            >
              {growthFormatted}
            </span>
          ) : null}
        </div>
        <div className="seller-sales-statistics-metric-card__chart">
          <SellerSalesTrendChart gradientId={gradientId} tone={tone} />
        </div>
      </div>
    </article>
  );
}
