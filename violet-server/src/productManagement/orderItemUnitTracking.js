...
/**
 * SellerSoldItem sync: mijozga sotilmagan donalar.
 * returned_to_seller ham — Ajdaniya qaytarish (no_answer «Sotildi» allaqachon
 * units[] ni delivered qiladi yoki recordSalesOnDelivery alohida yozadi).
 */
function isUnitExcludedFromSoldSync(item, unitIndexRaw) {
  const status = resolveUnitTrackingStatus(item, unitIndexRaw);
  if (status === "unavailable" || status === "cancelled") return true;
