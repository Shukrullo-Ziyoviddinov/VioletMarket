...
const DEFAULT_TOP_SILLERS_LIMIT = 10;
 ...
 */
function compareTopSillers(a, b) {
...
async function buildTopSillersFromDatabase({
  limit = DEFAULT_TOP_SILLERS_LIMIT,
  onlyActive = true,
  ...
  const effectiveLimit = Math.max(1, Number(limit) || DEFAULT_TOP_SILLERS_LIMIT);
  ...
  return rankTopSillers(input).slice(0, effectiveLimit);
}
