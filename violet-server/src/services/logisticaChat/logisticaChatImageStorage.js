...
function createLogisticaChatImageFilename() {
  const random = Math.random().toString(36).slice(2, 8);
  return `logistica-chat-${Date.now()}-${random}.jpg`;
}

function saveLogisticaChatImage(imageBase64) {
  ensureUploadRoot();
  const buffer = parseJpegBase64(imageBase64);
  const filename = createLogisticaChatImageFilename();
...
function isManagedLogisticaChatImage(publicPath) {
  const value = String(publicPath || "").trim();
  if (!value.startsWith("/uploads/")) return false;
  const filename = path.basename(value);
  return /^logistica-chat-\d+-[a-z0-9]+\.jpe?g$/i.test(filename);
}

function deleteManagedLogisticaChatImage(publicPath) {
  if (!isManagedLogisticaChatImage(publicPath)) return;
