...
function createSupportChatImageFilename() {
  const random = Math.random().toString(36).slice(2, 8);
  return `support-chat-${Date.now()}-${random}.jpg`;
}

function saveSupportChatImage(imageBase64) {
  ensureUploadRoot();
  const buffer = parseJpegBase64(imageBase64);
  const filename = createSupportChatImageFilename();
...
function isManagedSupportChatImage(publicPath) {
  const value = String(publicPath || "").trim();
  if (!value.startsWith("/uploads/")) return false;
  const filename = path.basename(value);
  return /^support-chat-\d+-[a-z0-9]+\.jpe?g$/i.test(filename);
}

function deleteManagedSupportChatImage(publicPath) {
  if (!isManagedSupportChatImage(publicPath)) return;
