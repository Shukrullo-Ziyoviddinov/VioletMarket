...
const { backfillBrandCategoryProductSalesFromOrders } = require("../../productManagement/recordBrandCategoryProductSales");
const {
  addDaysToDateKey,
  getIsoWeekStart,
  parseMonthKey,
...
function buildPeriodMatch(period, filters) {
  const match = { brandCategories: { $ne: "" } };
  if (period === "week") {
...
async function buildBrandCategorySalesStatistics(query = {}) {
  await backfillBrandCategoryProductSalesFromOrders();
  ...
  const rows = await BrandCategoryProductSale.aggregate([
    { $match: match },
    {
      $group: {
        _id: "$brandCategories",
  ...
  return {
    ...
    filters,
    totalQuantity,
    brands: buildBrandCategoryRows(rows, totalQuantity, metadata),
  };
}

module.exports = {
  buildBrandCategorySalesStatistics,
};
