...
const ImageBanner = ({ images = [] }) => {
    ...
    useEffect(() => {
        ...
        const slideWidth = slidesRef.current.querySelector('.manga-image')?.offsetWidth || 0;
                ...
                <ul className="manga-slides" ref={slidesRef}>
                    ...
                    {images.map((image, index) => (
                        <li 
                            ...
                            className={`manga-image ${getSlideClass(index - images.length)}`}
                            aria-hidden="true"
                            onClick={() => handleSlideClick(image)}
                            role={image.link ? 'button' : undefined}
                        >
                            <img 
                                src={normalizeImagePath(getLocalizedText(image.src, lang) || image.src)} 
                                alt={`Banner ${index + 1}`}
                                draggable={false}
                                onError={(e) => {
                    ...
                    {images.map((image, index) => (
                        <li 
                            key={image.id || index}
                            className={`manga-image ${getSlideClass(index)}`}
